...
class OTPScreen extends StatefulWidget with ChangeNotifier {
  String name;
  String password;
  String phoneNumber;

  OTPScreen(this.name, this.password, this.phoneNumber);
  @override
...
class _OTPScreenState extends State<OTPScreen> {
          ...
          verificationCompleted:
            ...
            await _auth.signInWithCredential(phoneAuthCredential).then((value) {
              ...
              _postToServerCreateUserByPhone(
                  uname, passwords, phone, otp, otpID, true);
            }).catchError((onError) {});
          },
  ...
  Future onCompletedOTP(v) async {
    try {
      await _auth
        ...
        var otpID = value.user?.uid;
        var passwords = widget.password;
        await _postToServerCreateUserByPhone(
            uname, passwords, phone, otp, otpID, true);
      }).catchError((onError) {
  ...
  //create user login
  Future _postToServerCreateUserByPhone(
      unameOTP, passwords, phoneOTP, otp, otpID, termAndCondition) async {
    setState(() {
      _isLoading = true;
    });
    var uname = unameOTP;
